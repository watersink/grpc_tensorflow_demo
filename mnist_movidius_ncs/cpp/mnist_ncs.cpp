...
const int networkDim = 28;
float networkMean[] = {127.5, 127.5, 127.5};
...
half *LoadImage(const char *path, int reqsize, float *mean)
{
	...
	half *imgfp16;
        ...
        //opencv method
        cv::Mat im = cv::imread(IMAGE_FILE_NAME,1);
        width = im.cols;
        height = im.rows;
        cp = im.channels();
	img = im.data;
